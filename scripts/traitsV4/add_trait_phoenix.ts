...
async function main() {
    ...
    const AllData = [];
    for(let i = dataStart; i < dataEND; i++) {
        if( i < largestRange.start || i > largestRange.end)  {
            if(traitData.includes(i)) {
                ExtraData.push(i);
            } 
        }
    ...
    let gasPrice = BigNumber.from(200000000000);
    let gasCost = BigNumber.from(0);

    // const gasCostEstimate = await ECRegistry.estimateGas.updateTrait(
    //     traitId,        // id
    //     "Phoenix",      // name
    //     ZERO_ADDRESS,   // implementer
    //     2,              // TraitType
    //     1000,           // TraitStart
    //     2562            // TraitEnd
    // );
    ...
    // update Phoenix
    // let tx = await ECRegistry.updateTrait(
    //     traitId,        // id
    //     "Phoenix",      // name
    //     ZERO_ADDRESS,   // implementer
    //     2,              // TraitType
    ...
    for (let k = 0; k < chunks.length; k++) {
        ...
        const tx = await ECRegistry.setData(traitId, indexes, values);  // , { gasLimit: 1000000}
        let txReceit = await tx.wait();
        gasCost = gasCost.add(0);

        console.log("txn:", txn, "traitId:", traitId, "chunk", k+1, "of", chunks.length,
         "gas:", 0, txReceit.cumulativeGasUsed.toNumber(), txReceit.transactionHash);
